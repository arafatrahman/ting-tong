...
$email = escape_string($post['verify-email']);
...
if (!empty($email)) {
    ...
    if (db_query_numrow($res) > 0) {
        ...
        $link = "http://localhost/my-chat-app/verify-success.php?email='.$email.'&hash='.$hash.'";

        $verifyStatus = $row['verify'];
        if (empty($verifyStatus)) {

            sendVerifyMail($email,$link,$name);
        }
    } else {
...
function sendVerifyMail($email,$link,$name)
{
    ...
    $mail->Subject = "Ting Tong | Verification";
    $mail->Body = '<head>
...
padding: 0 35px 40px;">
	...
	margin-bottom: 0;
">Hi '.$name.',</h1>
							<p class="near_title last" style="margin-top: 10px;margin-bottom: 0;">Please verify that your email address is '.$email.', and that you entered it when signing up for Ting Tong.</p>
...
background: #F4FBF9;">
    ...
    border: none !important;
padding: 20px;">
							<p class="marginless" style="margin: 0;">Happy carpooling, <br>The Ting Tong Team</p>
						</td>
					</tr>
				</tbody></table>
			</td>
...
">
	...
	line-height: 1.5;
">
				If you did not enter this email address when signing up for Ting Tong service, disregard this message.<br><br>
				© 2017 Google Inc. 1600 Amphitheatre Parkway, Mountain View, CA 94043
<br><br>

This is a mandatory service email from Ting Tong.
</p>
			</td>
		</tr>
	</tbody></table>
